...
import numpy as np
import cv2
from modules.utils.face_detect import nms, distance2bbox, distance2kps, custom_resize
from modules.configs import SCRFDConfig
from modules.model_server import TritonClient


class SCRFD:
	def __init__(self, config: SCRFDConfig, triton_client: TritonClient) -> None:
		self.config = config
	...
	def detect(self, img, input_size = None, max_num=0, metric='default'):
		...
		if max_num > 0 and det.shape[0] > max_num:
			...
			if kpss is not None:
				kpss = kpss[bindex, :]
		print(f"[{self.__class__.__name__}][{self.config.model_name}]: Face Detection finished.")
		return det, kpss
